...
```powershell
python scripts\sdd.py --project E:\path\to\project compete --task E:\path\to\task.md --profile generic-hosted
```
...
```powershell
python scripts\sdd.py --project E:\path\to\project rehearse-recovery --task E:\path\to\task.md --profile generic-hosted
```
...
```sh
python scripts/sdd.py --project /path/to/project compete --task /path/to/task.md --profile generic-hosted
```

演练模式：

```sh
python scripts/sdd.py --project /path/to/project rehearse-recovery --task /path/to/task.md --profile generic-hosted
```
...
正常使用应优先通过 `autonomous-sdd` Agent 入口触发。以下命令主要用于开发、调试和故障排查：
